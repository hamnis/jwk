...
object circe {
  implicit val byteVectorDecoder: Codec[ByteVector] =
    Codec.from(
      Decoder[String].emap(s =>
        ByteVector.fromBase64Descriptive(s, Bases.Alphabets.Base64Url).orElse(ByteVector.fromBase64Descriptive(s))
      ),
  ...
  implicit val curveDecoder: Codec[EllipticCurve.Curve] = Codec.from(
    Decoder[String].emap { alg =>
  ...
  implicit val x509Decoder: Decoder[Option[X509]] = Decoder.instance { c =>
    ...
    } yield x5u.map(url => X509Url(url, x5t, x5t256)).orElse(x5c.map(list => X509Chain(list, x5t, x5t256)))
  }
  ...
  implicit val jwkSetEncoder: Encoder[JwkSet] = Encoder.instance(set =>
    Json.obj(
